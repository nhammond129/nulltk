...
class RadialGauge(tk.Label, Gauge):
	...
	def _draw_base(self):
		...
		if self.stripethickness > 0:
			for theta in range(self.arcoffset, self.arcrange + self.arcoffset, self.stripethickness):
				draw.arc((20, 20, self.metersize*5 - 20, self.metersize*5 - 20),
						theta, theta + self.stripethickness - 1, self.meterbackground.as_hex(), self.meterthickness * 5)
		# solid
		else:
			draw.arc((20, 20, self.metersize*5 - 20, self.metersize*5 - 20),
					self.arcoffset, self.arcrange + self.arcoffset, self.meterbackground.as_hex(), self.meterthickness * 5)
	...
	def draw_meter(self, *args):
		...
		draw.text((self.metersize*2.75, self.metersize*2.5), f"{self.amountused}", anchor='rs', font=self.valuefont, fill=self.meterforeground.as_hex())
		draw.text((self.metersize*2.75, self.metersize*2.5), f"{self.unitstext}", anchor='ls', font=self.unitsfont, fill=self.meterforeground.darker(0.25).as_hex())
		draw.text((self.metersize*2.5, self.metersize*2.5), f"{self.labeltext}", anchor='ma', font=self.labelfont, fill=self.meterforeground.darker(0.25).as_hex())
	...
	def _draw_meter_solid(self, draw):
		...
		if self.wedgesize > 0:
			draw.arc((20, 20, self.metersize*5 - 20, self.metersize*5 - 20),
					 ...
					 self.meterforeground.as_hex(), self.meterthickness * 5)
		else:
			draw.arc((20, 20, self.metersize*5 - 20, self.metersize*5 - 20),
					self.arcoffset, self.meter_value(), self.meterforeground.as_hex(), self.meterthickness * 5)
	...
	def _draw_meter_striped(self, draw):
		...
		if self.wedgesize > 0:
			...
			for theta in range(self.arcoffset, self.meter_value() - 1, self.stripethickness):
				draw.arc((20, 20, self.metersize*5 - 20, self.metersize*5 - 20),
						 theta, theta + self.stripethickness - 1, self.meterforeground.as_hex(), self.meterthickness * 5)
